    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.augment:
            ...
            if random.random() < 0.5:
                ...
                # Convert mask to PIL Image for horizontal flip
                if isinstance(mask, np.ndarray):
                    mask_pil = Image.fromarray(mask.astype(np.uint8))
                    mask_pil = T.functional.hflip(mask_pil)
                    mask = np.array(mask_pil)
                elif isinstance(mask, torch.Tensor):
                    # If mask is already a tensor, we can use functional.hflip with tensor input
    ...
    def _load_image_and_annotations(
        ...
        for ann in anns:
            ...
            if 'segmentation' in ann:
                ...
                # Use actual label value (0-indexed) - not label+1
                mask[seg_mask] = label  # Class index starts from 0
